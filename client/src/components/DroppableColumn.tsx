...
export function DroppableColumn({ id, title, children, footer }: { id: string; title: string; children: ReactNode; footer?: ReactNode }) {
  ...
  return (
    <section ref={setNodeRef} className={`flex min-h-80 max-h-[calc(100vh-15rem)] flex-col rounded-xl border border-slate-200 bg-white p-4 shadow-sm transition ${isOver ? "ring-2 ring-sky-300" : ""}`}>
